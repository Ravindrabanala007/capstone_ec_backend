...
router.put('/update/:productId', protect, async (req, res) => {
  ...
  try {
    ...
    const product = await Product.findById(productId);
    if (!product) {
      return res.status(404).json({ message: 'Product associated with this cart item not found' });
    }

    if (qty > product.countInStock) {
      return res.status(400).json({ message: `Quantity exceeds available stock (${product.countInStock})` });
    }
